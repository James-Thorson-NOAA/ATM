...
Type objective_function<Type>::operator() ()
{
  ...
  array<Type> like_satellite_ig( n_i, n_g );
  array<Type> like_conventional_hg( n_h, n_g );
  ...
  for( int i=0; i<n_i; i++ ){
    for( int g=0; g<n_g; g++ ){
      like_satellite_ig(i,g) = prob_satellite_igt(i,g,satellite_iz(i,3));
    }
    nll_i(i) = -1 * log( like_satellite_ig(i,satellite_iz(i,1)) );
  }
  ...
  for( int h=0; h<n_h; h++ ){
    tmpsum = 0;
    for( int g=0; g<n_g; g++ ){
      tmpsum += Eprime_guy(g,uy_tz(conventional_hz(h,3),0),uy_tz(conventional_hz(h,3),1)) * prob_conventional_hgt(h,g,conventional_hz(h,3));
    }
    for( int g=0; g<n_g; g++ ){
  ...
  REPORT( like_satellite_ig );
  REPORT( like_conventional_hg );
