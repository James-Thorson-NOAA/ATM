...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(power_prime_e);
  ...
  if( n_j > 0 ){
    ...
    vector<Type> phi_e = exp(ln_phi_e);
    vector<Type> power_e = 1.0 + invlogit( power_prime_e );
    vector<Type> bhat_j( n_j );
    for( int j=0; j<n_j; j++ ){
      bhat_j(j) = exp(ln_d_st(g_j(j),t_j(j)));
      for( int m=0; m<n_m; m++ ){
        bhat_j(j) *= Q_jm(j,m) * lambda_m(m);
      }
      nll_j(j) = -1 * dtweedie( b_j(j), bhat_j(j), phi_e(e_j(j)), power_e(e_j(j)), true );
      SIMULATE{
        b_j(j) = rtweedie( bhat_j(j), phi_e(e_j(j)), power_e(e_j(j)) );   // Defined above
      }
      REPORT( bhat_j );
    }
  }
