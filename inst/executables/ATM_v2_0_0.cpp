...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(ln_kappa);
  PARAMETER(ln_sigma_omega);
  PARAMETER(ln_sigma_epsilon);
  PARAMETER(ln_phi);
  PARAMETER(power_prime);
  PARAMETER_VECTOR(Beta_t);
  PARAMETER_VECTOR(Omegainput_s);      // Expectation
  PARAMETER_ARRAY(Epsiloninput_st);   // Annual variation
  ...
  vector<Type> Omega_s( n_s );
  array<Type> Epsilon_st( n_s, n_t );
  array<Type> Epsiloninput_mean_st( n_s, n_t );
  Epsiloninput_mean_st.setZero();
  jnll += gmrf_Q( Omegainput_s );
  for( int t=0; t<n_t; t++ ){
    jnll += gmrf_Q( Epsiloninput_st.col(t) - Epsiloninput_mean_st.col(t) );
  }
  Omega_s = Omegainput_s * exp(ln_sigma_omega) / exp(logtau);
  Epsilon_st = Epsiloninput_st * exp(ln_sigma_epsilon) / exp(logtau);

  // Assemble density
  matrix<Type> d_gt( n_g, n_t );
  for( int g=0; g<n_g; g++ ){
  for( int t=0; t<n_t; t++ ){
    d_gt(g,t) = exp( Beta_t(t) + Omega_s(g) + Epsilon_st(g,t) );
  }}
  ...
  for( int j=0; j<n_j; j++ ){
    jnll -= dtweedie( b_j(j), d_gt(g_j(j),t_j(j)), phi, power, true );
  }
